  ...
  ({ script, startListening, faceV3CallbackRef, ...props }, ref) => {
    ...
    function startVisemeAnimation() {
      ...
      function animate() {
        ...
        const now =
          performance.now() - audioStartTimeRef.current + SYNC_OFFSET_MS;

        // Debug: Log timing for first viseme
        if (visemeTimelineRef.current.length > 0 && now < 200) {
          console.log(
            `Time: ${now.toFixed(1)}ms, First viseme at: ${
              visemeTimelineRef.current[0].offset_ms
            }ms`
          );
        }
    ...
    async function fetchVisemeData(text) {
      ...
      try {
        const res = await fetch(
          `https://avatar-dev-api.dtskill.com/api/generate-viseme/?text=${encodeURIComponent(
            text
          )}`
        );
        ...
        audio.onended = () => {
          ...
          if (SCRIPT_MAP[script] !== "Completion") {
            startListening();
          }
    ...
    return (
      <group ref={group} {...props} dispose={null} scale={6}>
        ...
        <group scale={48} position={[0, -62, 0]}>
